    ...
    try:
        ...
        if not X_test.shape[0] == y_test.shape[0]:
            raise AssertionError(
                (X_test.shape[0],
                 y_test.shape[0]))
        logging.info('SUCCESS: dependent and independent training and testing data have equal lengths')         
    except AssertionError as err:
        logging.error(
...
def test_metrics(compute_model_metrics):
    try:
        ...
        logging.info('SUCCESS: predictions and output test data have equal lengths')         
    except AssertionError as err:
        logging.error(
...
def model_slicing(data):
    ...
    for cat in cat_features:
        for cls in test[cat].unique():
            ...
            X_test_temp, y_test_temp, _, _ =process_data(df_temp,categorical_features=cat_features,label="salary", encoder=encoder, lb=lb, training=False)
            y_preds = model.predict(X_test_temp)
            precision_temp, recall_temp, fbeta_temp = compute_model_metrics(y_test_temp, y_preds)
            results = "[%s->%s] Precision: %s " \
                   "Recall: %s FBeta: %s" % (cat, cls, precision_temp, recall_temp, fbeta_temp)
...
if __name__ == '__main__':
    ...
    X_test, y_test, encoder_t, lb_t = process_data(test, categorical_features=cat_features, label="salary", training=False, encoder=encoder, lb=lb)
